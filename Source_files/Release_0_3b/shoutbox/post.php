...
$errorcodes = array("","You need to put a username and a message in the boxes below.","Did not supply a username.","Did not supply a Message to post.","Stop trying to impersonate the admin!");

include"swear_filter.php";

if (isset($_POST['submit']))
{
	$file = 'content.txt';
	$name = msgfilter(trim($_POST['name']));
	...
	if($name == "" && $message == ""){
		...
		$errorcode = 3;
	}elseif(strtolower($name) == strtolower($adminname)){
		$errorcode = 4;
	}else{
		$errorcode = 0;
	}

	if(strtolower($name) == strtolower($secretword)){
...
?>
				<p><img src="image.php" alt="This shoutbox is an image, navigate to /content.txt to have a screen reader access the messages." /></p>
