...
function postmsg($username,$message) {
	global $contentfile;
	$content = "[".date("H:ia")."]$username: $message\n" . file_get_contents($contentfile);
	writetofile($content);
}
...
function editform() {
	global $adminname, $adminpassword, $contentfile, $rssitemcount, $timestamps, $Xval, $Yval;
	...
	$form = "				<form method=\"post\" action=\"index.php\">
					...
					<p>Content file loaction: <input name=\"cfgContentfile\" type=\"text\" maxlength=\"50\" value=\"" . $contentfile . "\" /></p>
